...
- 42e6e10: Add several updates since `0.6.0`:
...
- b65f56c: Improve JWT audience validation in `client.verify()` while preserving existing behavior.
...
- e768774: Add a Cloudflare D1 storage adapter for OpenAuth deployments.

## 0.4.6

### Patch Changes

- 41cd4ce: Relax Cloudflare KV namespace typing so `CloudflareStorage` accepts KV namespace types generated by `wrangler types`.
